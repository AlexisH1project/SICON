		...
		if($tamanio > 0){
				for($i=0; $i<$tamanio; $i++){
					...
					if($resImp = mysqli_query($conexion, $sqlImp)){
                        ...
                        $fila--;
                    	$objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A8:DN".$fila);
						$objPHPExcel->getActiveSheet()->getStyle("A8:DN".$fila)->applyFromArray($estiloTituloColumnas);
                // Write the file
